...
class DirectManipulationControl(DirectObject):
    def __init__(self):
        ...
        #turn this off to disable separate handles for scaling
        self.useSeparateScaleHandles = True
        self.actionEvents = [
...
##             ['<', self.objectHandles.multiplyScalingFactorBy, 0.5],
        ...
        self.defaultSkipFlags = DG.SKIP_HIDDEN | DG.SKIP_BACKFACE | DG.SKIP_WIDGET
...
##                         pass
                ...
                if entry:
                    ...
                    self.hitPtDist = Vec3(self.hitPt).length()
                    # Select it
                    direct.select(entry.getIntoNodePath().parent, direct.fShift)
                else:
                    direct.deselectAll()
        #elif self.mode == 'move':
    ...
    def gridSnapping(self, nodePath, offset):
        ...
        spacing = self.fPosSpacing if self.fPosSpacing else ShowBaseGlobal.direct.grid.gridSpacing

        if offsetX < 0.0:
            signX = -1.0
        else:
            signX = 1.0
        modX = math.fabs(offsetX) % spacing
        floorX = math.floor(math.fabs(offsetX) / spacing)
        if modX < spacing / 2.0:
            offsetX = signX * floorX * spacing
        else:
            offsetX = signX * (floorX + 1) * spacing

        if offsetY < 0.0:
            signY = -1.0
        else:
            signY = 1.0
        modY = math.fabs(offsetY) % spacing
        floorY = math.floor(math.fabs(offsetY) / spacing)
        if modY < spacing / 2.0:
            offsetY = signY * floorY * spacing
        else:
            offsetY = signY * (floorY + 1) * spacing

        if offsetZ < 0.0:
            signZ = -1.0
        else:
            signZ = 1.0
        modZ = math.fabs(offsetZ) % spacing
        floorZ = math.floor(math.fabs(offsetZ) / spacing)
        if modZ < spacing / 2.0:
            offsetZ = signZ * floorZ * spacing
        else:
            offsetZ = signZ * (floorZ + 1) * spacing
    ...
    def plantSelectedNodePath(self):
        ...
        entry = ShowBaseGlobal.direct.iRay.pickGeom(
            skipFlags = DG.SKIP_HIDDEN | DG.SKIP_BACKFACE | DG.SKIP_CAMERA | DG.SKIP_WIDGET)
...
class ObjectHandles(NodePath, DirectObject):
    def __init__(self, name='objectHandles'):
        ...
        self.scalingNode.setName(name)

        # Make sure that the object handles always show in front
        # of the camera.
        ...
        # Set the bitmask for the collisions so they can be selected.
        for collision in (self.xPostCollision, self.yPostCollision, self.zPostCollision,
                          self.xRingCollision, self.yRingCollision, self.zRingCollision,
                          self.xDiscCollision, self.yDiscCollision, self.zDiscCollision,
                          self.xScaleCollision, self.yScaleCollision, self.zScaleCollision):
            collision.node().setFromCollideMask(BitMask32.allOff())
            mask = BitMask32()
            mask.setWord(0x80000000)
            collision.node().setIntoCollideMask(mask)

        # name disc geoms so they can be added to unpickables
        self.xDisc.find("**/+GeomNode").setName('x-disc-geom')
        self.xDisc.find("**/+GeomNode").setTag('WidgetName', name)

        self.yDisc.find("**/+GeomNode").setName('y-disc-geom')
        self.yDisc.find("**/+GeomNode").setTag('WidgetName', name)

        self.zDisc.find("**/+GeomNode").setName('z-disc-geom')
        self.zDisc.find("**/+GeomNode").setTag('WidgetName', name)
