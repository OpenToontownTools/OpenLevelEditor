...
##             ]
    ...
    def mouseRotateStart(self, modifiers):
        ...
        if not base.imgui.isMouseCaptured() and self.useMayaCamControls: # use maya controls
            ...
            elif direct.gotAlt(modifiers) and direct.gotControl(modifiers) \
               and not direct.gotShift(modifiers):
                self.mouseFlyStart(modifiers)
            elif direct.gotAlt(modifiers) and direct.gotShift(modifiers) \
               and not direct.gotControl(modifiers):
                self.mouseDollyStart(modifiers)
    ...
    def mouseFlyStart(self, modifiers):
        ...
        if self.useMayaCamControls and direct.gotAlt(modifiers):          # alt is down, use maya controls
            # Hide the marker for this kind of motion
            self.coaMarker.hide()
            # Record time of start of mouse interaction
            self.startT = base.clock.getFrameTime()
