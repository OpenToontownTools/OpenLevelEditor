...
class GroupPanel(DirectObject.DirectObject):
    ...
    def __checkGroupStatus(self):
        ...
        if myMemberList:
            for avId in myMemberList:
                ...
                # In case you can't find the toon don't make the avatar button.
                if avatarButton:
    ...
    def __getAvatarButton(self, avId):
        ...
        if toon.battleId:
            ...
            # If local avatar is in battle hide the group panel
            if (avId == localAvatar.doId):
                self.__forceHide()
        else:
            if (avId == self.leaderId):
                # Toon is the group leader - change image to buttonImage to leaderImage
                buttonImage = self.leaderButtonImage
            # If local avatar is not in battle show the group panel
            if (avId == localAvatar.doId):
                self.__forceShow()
        return DirectButton(
            parent = self.frame,
